...
@app.post("/set_banner")
def set_banner(payload: dict, db: Session = Depends(get_db)):
    ...
    if account.role != "dev":
        return {"success": False, "error": "Only developers can update the banner."}

    config = db.query(models.ServerConfig).first()
    if not config:
        config = models.ServerConfig(login_banner=message)
        db.add(config)
    else:
        config.login_banner = message
...
def collect_materials(payload: dict, db: Session = Depends(get_db)):
    ...
    elapsed = (now - player.gathering_start_time).total_seconds()
    units = max(1, int(elapsed // 5))  # 1 unit per 5 seconds
    ...
    message = f"You collected {total_items} x {get_item_name(best_item_id)} after {elapsed} second(s) of {activity}."
